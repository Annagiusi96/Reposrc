    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            while (reader.Read())
            {
                ...
                regione.Nome = reader.GetString(1);
            ...
            RegioniDAO rDao = new();
            List<Regione> regioni = rDao.FindAll();
            //Console.WriteLine($"Trovate {regioni.Count} regioni");
            ...
            Console.WriteLine(rDao.Count());

            int regioneId = 15;
            Console.WriteLine($"alla posizione {regioneId}: {rDao.FindById(regioneId)}");


            int regioneId = 35;
            Console.WriteLine($"alla posizione {regioneId}: {rDao.FindById(regioneId)}"); //regione vuota
